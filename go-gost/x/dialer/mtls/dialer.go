...
func (d *mtlsDialer) Dial(ctx context.Context, addr string, opts ...dialer.DialOption) (conn net.Conn, err error) {
	...
	if session != nil {
		...
		if closed || rotate {
			if rotate {
				d.logger.Debugf("mtls: rotating idle mux session addr=%s age=%v", addr, time.Since(session.createdAt))
			}
			_ = session.Close()
...
func (d *mtlsDialer) initSession(ctx context.Context, conn net.Conn) (*muxSession, error) {
	...
	return &muxSession{conn: conn, session: session, createdAt: time.Now()}, nil
}
